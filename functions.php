...
add_action( 'wp_enqueue_scripts', 'taiyo_scripts');

/**
 * Add admin styles and scripts
 */
function taiyo_admin_style() {
	wp_enqueue_style( 'main-admin-style', get_template_directory_uri() . '/assets/css/admin.css', [], false, 'all' );
}
add_action( 'admin_enqueue_scripts', 'taiyo_admin_style' );


function taiyo_config(){
  register_nav_menus(
    array(
  'taiyo_main_menu' => 'Taiyo Main Menu',
    )
  );
}

add_action('after_setup_theme', 'taiyo_config', 0);
